...
public class Main3 {
    ...
    //int curPos, int ifZero, int curLimit, int sum
    private static long[][][][] dp = new long[20][2][2][20];
    ...
    private static long dfs(int curPos, int curDigit, int ifZero, int curLimit, int sum) {
         ...
         * 这里要加ifZero != 1 && curLimit != 1限制，不然超时
         */
        if (ifZero != 1 && curLimit != 1 && dp[curPos][ifZero][curLimit][sum] != 0)
            return dp[curPos][ifZero][curLimit][sum];
        ...
        if (ifZero != 1 && curLimit != 1)
            dp[curPos][ifZero][curLimit][sum] = cnt;
        return cnt;
    }
}
