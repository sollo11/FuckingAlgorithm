...
public class Solution {
     ...
     */
    private void dfs(String to_spilt, int all_len, int cur_spilt_cnt, String cur_split_path){
        if("".equals(to_spilt)&&cur_split_path.split("\\.").length==5){
            res.add(cur_split_path.substring(1));return;
        }
        //三种选择，截1,2,3个
        for (int i=1;i<=(to_spilt.length()>=3?3:to_spilt.length());i++){
            ...
            dfs(right,all_len,cur_spilt_cnt,cur_split_path);
