package 数论系列.快速幂模板.应用.超级次方;

/**
 * @Author: Jack
 * @Date: 2020/6/11 11:35
 * @Description:
 * 次幂的位数可能超过几百位，所以只能通过递归方式的快速幂求解，只不过这里的求余需要进行一定的优化
 ...
 * @限制: 计算 a^b 对 1337 取模，a 是一个正整数，b 是一个非常大的正整数且会以数组形式给出。
 * @Level: 
 */
public class Solution {
    ...
    public int superPow(int a, int[] b) {
       ...
       for (int i = 0; i < b.length; i++)
           res = fastPow(res, 10) * fastPow(a, b[i]) % MOD; //下一位的res为，上一位的res的十次方再乘以a的该位次方再对1337取余
       return res;
    }
    private int fastPow(int x, int n) {
        ...
        if (n == 1) return x % MOD;
        if ((n & 1) == 1)
            return ((x % MOD) * (fastPow(x, n - 1) % MOD)) % MOD; //优化求余
        else {
            int tmp = fastPow(x, n / 2) % MOD;
            return tmp * tmp % MOD;
        }
    }
    ...
    public static void main(String[] args){
         ...
         int res = new Solution().superPow(a,b);
